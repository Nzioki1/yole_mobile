...
class TransferNotifier extends StateNotifier<TransferState> {
  ...
  Future<void> getQuote(
    ...
    try {
      ...
      state = state.copyWith(draft: updatedDraft, quote: quote, loading: false);

      _trackEvent(
        'transfer_quote_received',
        params: {
          'amount': quote.amount,
      ...
      _trackEvent('transfer_quote_failed', params: {'error': e.message});
    } catch (e) {
      state = state.copyWith(
        error: UnknownFailure(e.toString()),
        loading: false,
      );
      _trackEvent('transfer_quote_failed', params: {'error': e.toString()});
    }
  }
  ...
  Future<void> createTransfer() async {
    ...
    try {
      ...
      state = state.copyWith(redirect: redirect, isSubmitting: false);

      _trackEvent(
        'transfer_submit_success',
        params: {'order_tracking_id': redirect.orderTrackingId},
      );
    } on Failure catch (e) {
      state = state.copyWith(error: e, isSubmitting: false);
      _trackEvent('transfer_submit_failed', params: {'error': e.message});
    } catch (e) {
      state = state.copyWith(
        error: UnknownFailure(e.toString()),
        isSubmitting: false,
      );
      _trackEvent('transfer_submit_failed', params: {'error': e.toString()});
    }
  }

  Future<void> createPesapalTestPayment() async {
    if (state.draft == null) return;

    state = state.copyWith(isSubmitting: true, error: null);
    _trackEvent(
      'pesapal_test_payment_started',
      params: {
        'amount': state.draft!.amount,
        'recipient_id': state.draft!.recipientId,
      },
    ...
    try {
      final redirect = await repo.createPesapalTestPayment(
        state.draft!.amount.toString(),
        state.draft!.recipientCountry,
        state.draft!.phoneNumber,
      );
      ...
      _trackEvent(
        'pesapal_test_payment_success',
        params: {'order_tracking_id': redirect.orderTrackingId},
      );
    } on Failure catch (e) {
      state = state.copyWith(error: e, isSubmitting: false);
      _trackEvent('pesapal_test_payment_failed', params: {'error': e.message});
    } catch (e) {
      state = state.copyWith(
        error: UnknownFailure(e.toString()),
        isSubmitting: false,
  ...
  Future<String> getTransactionStatus(String orderTrackingId) async {
    try {
      ...
      _trackEvent(
        'transfer_status_checked',
        params: {'order_tracking_id': orderTrackingId, 'status': status},
      );
      return status;
    } on Failure catch (e) {
      _trackEvent('transfer_status_failed', params: {'error': e.message});
      rethrow;
    } catch (e) {
      _trackEvent('transfer_status_failed', params: {'error': e.toString()});
      rethrow;
    }
  }
